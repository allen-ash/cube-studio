...
from myapp.models.model_job import Repository, Images, Job_Template, Task, Pipeline, Workflow, Tfjob, Xgbjob, \
    RunHistory, Pytorchjob
...
from wtforms import BooleanField, IntegerField, StringField, SelectField, FloatField, DateField, DateTimeField, \
    SelectMultipleField, FormField, FieldList
...
from flask_appbuilder.fieldwidgets import BS3TextFieldWidget, BS3PasswordFieldWidget, DatePickerWidget, \
    DateTimePickerWidget, Select2ManyWidget, Select2Widget, BS3TextAreaFieldWidget
from myapp.forms import MyBS3TextAreaFieldWidget, MySelect2Widget, MyCodeArea, MyLineSeparatedListField, MyJSONField, \
    MyBS3TextFieldWidget, MySelectMultipleField
...
def dag_to_pipeline(pipeline, dbsession, **kwargs):
    ...
    pipeline_global_env = template_str(
        pipeline.global_env.strip()) if pipeline.global_env else ''  # 优先渲染，不然里面如果有date就可能存在不一致的问题
    ...
    def get_ops(task_name):
                ...
                if "(pvc)" in volume:
                    ...
                    ops = ops.add_volume(k8s_client.V1Volume(name=volumn_name,
                                                             persistent_volume_claim=k8s_client.V1PersistentVolumeClaimVolumeSource(
                                                                 claim_name=pvc_name))) \
                        .add_volume_mount(
                        k8s_client.V1VolumeMount(mount_path=os.path.join(mount, task.pipeline.created_by.username),
                                                 name=volumn_name, sub_path=task.pipeline.created_by.username))
                ...
                if "(memory)" in volume:
                    ...
                    ops = ops.add_volume(k8s_client.V1Volume(name=volumn_name,
                                                             empty_dir=k8s_client.V1EmptyDirVolumeSource(
                                                                 medium='Memory', size_limit='%sGi' % memory_size))) \
                        .add_volume_mount(k8s_client.V1VolumeMount(mount_path=mount, name=volumn_name))
        ...
        for selector in re.split(',|;|\n|\t', task.get_node_selector()):
            ...
            if '=' in selector:
                ops.add_node_selector_constraint(selector.strip().split('=')[0].strip(),
        ...
        resource_cpu = task.job_template.get_env('TASK_RESOURCE_CPU') if task.job_template.get_env(
            'TASK_RESOURCE_CPU') else task.resource_cpu
        resource_gpu = task.job_template.get_env('TASK_RESOURCE_GPU') if task.job_template.get_env(
            'TASK_RESOURCE_GPU') else task.resource_gpu
        resource_memory = task.job_template.get_env('TASK_RESOURCE_MEMORY') if task.job_template.get_env(
            'TASK_RESOURCE_MEMORY') else task.resource_memory
...
def upload_pipeline(pipeline_file, pipeline_name, kfp_host, pipeline_argo_id):
    ...
    if pipeline_argo:
        pipeline_argo_version = client.upload_pipeline_version(pipeline_package_path=pipeline_name + '.yaml',
                                                               pipeline_version_name=pipeline_name + "_version_at_" + datetime.datetime.now().strftime(
                                                                   '%Y-%m-%dT%H:%M:%S'), pipeline_id=pipeline_argo.id)
...
def run_pipeline(pipeline_file, pipeline_name, kfp_host, pipeline_argo_id, pipeline_argo_version_id):
    ...
    if not pipeline_argo_id or not pipeline_argo_version_id:
        pipeline_argo_id, pipeline_argo_version_id = upload_pipeline(pipeline_file, pipeline_name, kfp_host,
                                                                     pipeline_argo_id)  # 必须上传新版本
    ...
    # 直接使用pipeline最新的版本运行
    try:
        run = client.run_pipeline(experiment_id=experiment.id, pipeline_id=pipeline_argo_id,
                                  version_id=pipeline_argo_version_id,
                                  job_name=pipeline_name + "_version_at_" + datetime.datetime.now().strftime(
                                      '%Y-%m-%dT%H:%M:%S'))
...
class Pipeline_ModelView_Base():
    ...
    add_columns = ['project', 'name', 'describe', 'schedule_type', 'cron_time', 'depends_on_past', 'max_active_runs',
    ...
    def pre_add(self, item):
        ...
        item.parameter = json.dumps({"cronjob_start_time": datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')},
    ...
    def pre_update(self, item):
        ...
        if (item.schedule_type == 'crontab' and self.src_item_json.get("schedule_type") == 'once') or (
                item.cron_time != self.src_item_json.get("cron_time", '')):
    ...
    def run_pipeline(self, pipeline_id):
        ...
        print('success upload and run pipeline %s,pipeline_argo_id %s, version_id %s,run_id %s ' % (
    ...
    def web_monitoring(self, pipeline_id):
        ...
        if pipeline.run_id:
            url = pipeline.project.cluster.get('GRAFANA_HOST', '').strip('/') + conf.get(
                'GRAFANA_TASK_PATH') + pipeline.name
            return redirect(url)
            # data = {
    ...
    def web_pod(self, pipeline_id):
        pipeline = db.session.query(Pipeline).filter_by(id=pipeline_id).first()
        data = {
            "url": pipeline.project.cluster.get('K8S_DASHBOARD_CLUSTER', '') + '#/search?namespace=%s&q=%s' % (
    ...
    def web_runhistory(self, pipeline_id):
        url = conf.get('MODEL_URLS', {}).get('runhistory', '') + '?filter=' + urllib.parse.quote(
    ...
    def web_workflow(self, pipeline_id):
        url = conf.get('MODEL_URLS', {}).get('workflow', '') + '?filter=' + urllib.parse.quote(
            json.dumps([{"key": "labels", "value": '"pipeline-id": "%s"' % pipeline_id}], ensure_ascii=False))
        print(url)
        return redirect(url)
...
appbuilder.add_view(Pipeline_ModelView, "任务流", href="/pipeline_modelview/list/?_flt_0_created_by=", icon='fa-sitemap',
                    category='训练')
...
class Pipeline_ModelView_Api(Pipeline_ModelView_Base, MyappModelRestApi):
    ...
    list_columns = ['id', 'project', 'pipeline_url', 'creator', 'modified']
    add_columns = ['project', 'name', 'describe', 'schedule_type', 'cron_time', 'depends_on_past', 'max_active_runs',
                   'parallelism', 'global_env', 'alert_status', 'expand']
    edit_columns = ['project', 'name', 'describe', 'schedule_type', 'cron_time', 'depends_on_past', 'max_active_runs',
                    'parallelism', 'dag_json', 'global_env', 'alert_status', 'expand', 'created_by']

...
class Training_Model_ModelView_Base():
    ...
    order_columns = ['id']
    list_columns = ['name','project_url','pipeline_url','version','creator','status','modified','deploy']

    cols_width={
        "name":{"type": "ellip2", "width": 300},
        "project_url": {"type": "ellip2", "width": 150},
            ...
            try:
                ...
                try:
                    if os.path.exists(host_save_dir):
                        shutil.rmtree(host_save_dir, ignore_errors=True)
    ...
    def pre_delete(self,item):
        # 删除测试环境
        if item.status == 'online' or item.status=='test':
            self.delete_deploy(item.id)
        ...
        if os.path.exists(host_model_path):
            ...
            if os.path.isdir(host_model_path):
                ...
                if os.path.exists(des_saved_model_path):
                    shutil.rmtree(des_saved_model_path, ignore_errors=True)
